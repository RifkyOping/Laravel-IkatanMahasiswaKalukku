                ...
                <form action="{{ route('admin.galleries.update', $gallery->id) }}" method="POST"
                    ...
                    <div>
                        ...
                        <textarea name="content" id="content" rows="4" required
                            class="block w-full border-gray-200 focus:border-imk-400 focus:ring-imk-400 rounded-2xl shadow-sm text-gray-700 p-4 bg-gray-50 hover:bg-white transition-colors">{{ old('content', $gallery->content) }}</textarea>
                        @error('content')
                            <p class="text-red-500 text-sm mt-2 flex items-center gap-1"><svg class="w-4 h-4" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="date" class="block text-sm font-bold text-gray-700 mb-2">Tanggal Pelaksanaan /
                                Mulai</label>
                            <input type="date" name="date" id="date" value="{{ old('date', $gallery->date) }}" required
                                class="block w-full border-gray-200 focus:border-imk-400 focus:ring-imk-400 rounded-2xl shadow-sm text-gray-700 p-4 bg-gray-50 hover:bg-white transition-colors">
                            @error('date')
                        ...
                        <div
                            ...
                            <div class="space-y-1 text-center">
                                ...
                                <p class="text-xs text-gray-500 mt-2">Biarkan kosong jika tidak ingin mengubah
                                    foto.<br>Jika diunggah, akan menimpa seluruh foto lama.</p>
                            </div>
                        </div>
                        @error('images')
                            <p class="text-red-500 text-sm mt-2 flex items-center gap-1"><svg class="w-4 h-4" fill="none"
