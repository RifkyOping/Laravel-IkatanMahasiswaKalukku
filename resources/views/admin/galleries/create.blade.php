                ...
                <form action="{{ route('admin.galleries.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title') }}" required
                            ...
                            placeholder="Masukkan judul acara atau kegiatan">
                        @error('title')
                    ...
                    <div>
                        <label for="content" class="block text-sm font-bold text-gray-700 mb-2">Deskripsi
                            Singkat</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="date" class="block text-sm font-bold text-gray-700 mb-2">Tanggal Pelaksanaan /
                                Mulai</label>
                            <input type="date" name="date" id="date" value="{{ old('date') }}" required
                                class="block w-full border-gray-200 focus:border-imk-400 focus:ring-imk-400 rounded-2xl shadow-sm text-gray-700 p-4 bg-gray-50 hover:bg-white transition-colors">
                            @error('date')
                                <p class="text-red-500 text-sm mt-2 flex items-center gap-1"><svg class="w-4 h-4"
                        ...
                        <div>
                            <label for="end_date" class="block text-sm font-bold text-gray-700 mb-2">Tanggal Selesai
                    ...
                    <div>
                        <label for="drive_link" class="block text-sm font-bold text-gray-700 mb-2">Link Drive
                    ...
                    <div>
                        <label for="images" class="block text-sm font-bold text-gray-700 mb-2">Foto Dokumentasi
                            (Maksimal 3 Gambar)</label>
                        <div
                            ...
                            <div class="space-y-1 text-center">
                                ...
                                <p class="text-xs text-gray-500 mt-2">Bisa memilih lebih dari satu file (Max 3).<br>PNG,
                                    JPG up to 2MB per file.</p>
                            </div>
                        </div>
                        @error('images')
                            <p class="text-red-500 text-sm mt-2 flex items-center gap-1"><svg class="w-4 h-4" fill="none"
                    ...
                    <div class="pt-4 flex items-center gap-4 border-t border-gray-100">
                        <button type="submit"
                            class="px-8 py-3 bg-imk-600 text-white font-bold rounded-full hover:bg-imk-700 shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-0.5">Simpan
                            Galeri</button>
                        <a href="{{ route('admin.galleries.index') }}"
