        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-[39px] shadow-xl p-10 md:p-12 relative overflow-hidden">
                <form action="{{ route('admin.attachments.update', $attachment->id) }}" method="POST"
                    ...
                    <div>
                        <label for="title" class="block text-sm font-bold text-gray-700 mb-2">Judul / Nama
                            Tampilan</label>
                    ...
                    <div>
                        ...
                        <div
                            class="p-4 bg-gray-50 rounded-2xl border border-gray-200 mb-4 flex items-center justify-between">
                            <span
                                class="text-gray-600 font-medium">{{ $attachment->original_name ?? 'File tidak diketahui' }}</span>
                            <a href="{{ route('lampiran.download', $attachment->id) }}"
                                class="text-imk-600 hover:text-imk-800 font-bold text-sm underline">Download</a>
                        </div>

                        <label for="file" class="block text-sm font-bold text-gray-700 mb-2">Ganti File Lampiran
                            (Opsional)</label>
                        <div
